...
class ImageDataset(Dataset):
    def __init__(self, csv_file, img_size):
        ...
        check_file = os.path.join('FF_check_face', 'c40_all.pkl')
        try:
    ...
    def __init__(self, csv_file, img_size, filter_size, test_set):
        ...
        with open(csv_file, newline='') as csvfile:
            ...
            for row in rows:
                ...
                '''
                try:
                    if num == face[img_path.split('/')[-4]][img_path.split('/')[-3]]:
                        self.img.append(img_path)
                        self.label.append(mylabel) 
                    elif  img_path.split('/')[-4] == "original":
                        self.img.append(img_path)
                        self.label.append(mylabel)
                    else:
                        pass
                        #print("Ignore this video: {} / face: {}.".format(video, num))
                except:
                    pass
                    #print("Ignore this video: {} / face: {}.".format(video, num))
                '''
